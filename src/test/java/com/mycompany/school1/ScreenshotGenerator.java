...
public class ScreenshotGenerator {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("ALL_DROPDOWN_SCREENSHOTS_SUCCESSFUL");
            System.exit(0);
    ...
    private static void captureRobot(Robot robot, JFrame frame, String fileName) {
        try {
            ...
            System.out.println("Robot captured: " + outFile.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static void captureRobotWithPopup(Robot robot, JFrame frame, String fileName) {
        try {
            Point p = frame.getLocationOnScreen();
            Dimension d = frame.getSize();
            int extraHeight = 120; // Captures expanded popup dropdown
            Rectangle rect = new Rectangle(p.x, p.y, d.width, d.height + extraHeight);
            BufferedImage img = robot.createScreenCapture(rect);
            File outFile = new File("assets/screenshots/" + fileName);
            ImageIO.write(img, "png", outFile);
            System.out.println("Robot captured popup: " + outFile.getAbsolutePath());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
